...
pair<double, double> triangle::bound(int dim) const {
    double l = vertex[0].e[dim], r = vertex[0].e[dim];
    for(int i=1;i<3;i++) {
        l = min(l, vertex[i][dim]);
        r = max(r, vertex[i][dim]);
    }
    return pair<double, double>(l, r);
}
